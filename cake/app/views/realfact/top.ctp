...
function move(initEnemy){
  if(initEnemy.delete_flag==1 && initEnemy.hit_flag==0){
    ...
    missEnemy(initEnemy);
  }else if(initEnemy.hit_flag==1 && initEnemy.genre_id == 1){
    hitEnemy(initEnemy);
  }else if(initEnemy.hit_flag==1 && initEnemy.genre_id == 2){
    hitEnemy(initEnemy);
  }else if(initEnemy.hit_flag==1 && initEnemy.genre_id == 3){
    hitEnemy(initEnemy);
  }else if(initEnemy.hit_flag==1 && initEnemy.genre_id == 4){
    outEnemy(initEnemy);
  }
}
...
function load_win_movie(){
  ...
  if(movie_y>=0){
    ...
    if(page_move_flag==0){
      page_move_flag = 1;
      location.href='http://bank.blamestitch.com/cake/realfact/win/<?php echo $quest_id; ?>/#header-menu/';
    }
  }
}
/*結果_失敗*/
function load_lose_movie(){
  ...
  if(movie_y>=0){
    ...
    if(page_move_flag==0){
      page_move_flag = 1;
      location.href='http://bank.blamestitch.com/cake/realfact/lose/<?php echo $quest_id; ?>/#header-menu/';
    }
  }
}
/*カード_ヒット_ミス*/
...
/*カード_一発アウト*/
function outEnemy(initEnemy){
  enemy_c_1 = new Image();
  enemy_c_1.src = "/img/game/2-2.png";
  enemy_c_2 = new Image();
  enemy_c_2.src = "/img/game/2-3.png";
  enemy_c_3 = new Image();
  enemy_c_3.src = "/img/game/2-4.png";
  enemy_c_4 = new Image();
  enemy_c_4.src = "/img/game/2-5.png";
  enemy_c_5 = new Image();
  enemy_c_5.src = "/img/game/2-5.png";
  movie_img = eval("enemy_a_" + initEnemy.a_img_count);
  ctx.drawImage(movie_img,initEnemy.x,initEnemy.y);
  initEnemy.c_img_count+=1;
  if(initEnemy.c_img_count>5){initEnemy.c_img_count=5;}
  if(initEnemy.c_img_count == 2 && initEnemy.dont_kill_flag == 1){
    //score-=1;
  }else if(initEnemy.c_img_count == 2 && initEnemy.dont_kill_flag == 0){
    score+=1;
    challenge_count+=10;
  }
}
